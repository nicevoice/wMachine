...
if not os.path.exists("data/" + ARGS_STORE):
	os.mkdir("data/" + ARGS_STORE)
if not os.path.exists("freqs/" + ARGS_STORE):
	os.mkdir("freqs/" + ARGS_STORE)
if not os.path.exists("input_parts/" + ARGS_STORE):
...
for uid in ARGS_UIDS:
	...
	IS_PRO = False
	# init PATH
	PATH_REAL = "http://cn.weibo.com/u/"+uid
	...
	try:
		for i in range(1,200):
			...
			if IS_PRO:
				request = urllib2.Request("http://cn.weibo.com/aj/mblog/mbloglist?uid="+uid+"&page="+str(i)+"&pagebar=0")
			else:
			...
			if IS_PRO:
				request = urllib2.Request("http://cn.weibo.com/aj/mblog/mbloglist?uid="+uid+"&page="+str(i)+"&pagebar=1")
			else:
